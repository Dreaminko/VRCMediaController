...
- **BLE Heart Rate**: Connects directly to devices that expose the Bluetooth SIG Heart Rate Service and appends live BPM to the Chatbox.
...
- **BLE 心率**：直接连接公开 Bluetooth SIG Heart Rate Service 的设备，并将实时 BPM 追加到聊天框。
...
- **BLE心拍数**：Bluetooth SIG Heart Rate Serviceを公開するデバイスへ直接接続し、BPMをチャットボックスへ追加します。
...
1. Make the sensor discoverable or pair it in Windows. Some watches expose heart-rate data only while a workout is active.
2. Enable **BLE Heart Rate** in VRCMediaController, click **Scan**, and select the device.
3. The device must expose the standard Heart Rate Service (`0x180D`) and Heart Rate Measurement characteristic (`0x2A37`). The app does not support vendor-private protocols.
